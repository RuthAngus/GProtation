...
inference, such as having highly degenerate or high-dimensional parameter spaces.
...
\naigrain\ simulated light curves, demonstrating that our results
...
Complicated surface spot patterns produce the non-sinusoidal quality,
and the finite lifetimes of these active regions and the presence of 
...
Considering that many stars likely have hundreds of spots, the number
of free parameters quickly becomes unwieldy, especially to explore
their posterior PDFs with MCMC.
...
non-sinusoidal.
In fact, as long as it is approximately periodic the ACF will 
display a peak at the rotation period, no matter the signal shape.
...
the data, and so not inherently probabilistic.
...
and be flexible enough to perfectly capture non-sinusoidal, QP behaviour.
A Gaussian process (GP) model fulfills these requirements. We use this GP 
model as the generative model at the backbone of a method to probabilistically 
...
This gives GPs their flexibility---they can model any time
...
Figure \ref{fig:GP_example} demonstrates how the light curve of KIC \kepexample\ 
may be modelled with a GP.
...
Exponential' (SE), defined as follows, 
produces an adequate fit to the KIC \kepexample\ light curve:
\begin{equation}
...
However, The SE kernel function is not a good model of the covariance 
in stellar light curves, nor a {\it useful} one for the problem of 
rotation period inference because it does not capture periodic behaviour.
In order to infer rotation periods it is thus necessary to use a periodic kernel
...
so slight deviations from positive definiteness can arise.
...
To infer a stellar rotation period from a light curve, we fit this QP-kernel
...
In this study, however, we explore the full posterior PDFs using MCMC.  While
this approach comes at a computational cost, such posterior exploration 
importantly provides a justified estimate of the uncertainty on rotation period.
