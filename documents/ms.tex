...
\includegraphics[width=6in, clip=true]{figures/comparison_emcee3_17_1.png}
...
The uncertainties published in the \citet{Mcquillan2014} catalogue were
calculated from the scatter in the positions of peaks in the ACF that are
separated by integer multiples of the period.
We show a comparison of our results to those of the \kepler\ Objects of
Interest (KOIS) \citet{Mcquillan2014} in order to demonstrate that our
technique works on real \kepler\ light curves, not just simulated ones.
These are perhaps not the ideal test subjects because many KOI light curves
have deep stellar occulatations and planetary transits which can look
like non-Gaussian noise.
Since Gaussian processes require that the data are Gaussian, this could have
detremental effects on the results.
Despite this however, our results are, in most cases, similar to those of
\citet{Mcquillan2014} indicating that our method still works well even in the
presence of transits and eclipses.
In many cases where there is a large discrepancy between our results and the
\citet{Mcquillan2014} results, either the variability is low amplitude or
frequent, deep eclipses/transits are present in the data.
In most other cases we expect our results to be the more accurate ones and the
discrepancy may be due to quasi-periodicity in the light curve.
...
method works well on real \kepler\ data and is, in most cases robust to the
presents of transits and eclipses in the light curves.

Although rotation period uncertainties calculated using the GP method are
better estimates than those calculated using the ACF method, they are still
imperfect estimates of the intrinsic uncertainties.
This is because the posterior PDF of the rotation period parameter is, in many
cases, multi-modal.
If it were possible to run an MCMC sampler for an infinite number of steps, we
would the entire posterior PDF would eventually be sampled, producing an
...
In these cases it is therefore misleading to report median values or 16th and
...
The main drawback of the GP method is the computational cost.
