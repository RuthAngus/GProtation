...
Although we follow the same overall ACF method in this paper, the exact
implementation of \citet{Mcquillan2013} code is slightly different and
therefore produces different results.
The MRD of their results relative to the true rotation periods is
\percenttelavivMAD\% which is smaller than the results produced by the GP
method: their results are more precise.
However, they produce a larger number of large outliers --- they predict
...
results, which is more sensitive to outliers: the RMS of the Tel Aviv results
is 25 days and the RMS of GP results is 7 days.
The Tel Aviv ACF results are therefore more precise but are catastrophically
wrong in a larger number of cases.
