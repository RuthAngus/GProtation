...
\newcommand{\lnacfMAD}{0.14}
\newcommand{\acfMAD}{2.36}
\newcommand{\percentacfMAD}{13.02}
\newcommand{\acfRMS}{26.09}

\newcommand{\lnpgramMAD}{0.05}
\newcommand{\pgramMAD}{1.13}
\newcommand{\percentpgramMAD}{4.70}
\newcommand{\pgramRMS}{18.98}

\newcommand{\lngpMADnp}{0.02}
\newcommand{\gpMADnp}{0.48}
\newcommand{\percentgpMADnp}{2.45}
\newcommand{\gpRMSnp}{7.48}

\newcommand{\lngpMAD}{0.02}
\newcommand{\gpMAD}{0.39}
\newcommand{\percentgpMAD}{1.96}
\newcommand{\gpRMS}{7.12}

\newcommand{\lntelavivMAD}{0.02}
\newcommand{\telavivMAD}{0.34}
\newcommand{\percenttelavivMAD}{1.75}
\newcommand{\telavivRMS}{25.09}
...
The code is available at \url{https://github.com/RuthAngus/GProtation/} .
...
We also compare our results to those of the Tel Aviv team in
\citet{Aigrain2015} who use the ACF method described in \citet{Mcquillan2013}.
Although we follow the same general ACF method in this paper, the exact
implementation of the \citet{Mcquillan2013} code is slightly different and
produces different results.
The MRD of their periods, relative to the true rotation periods, is
\percenttelavivMAD\% --- smaller than the results produced by the GP
method.
However, their results contain a larger number of extreme outliers --- they
predict
periods that are significantly different from the true period in more cases
than the GP method.
This is revealed by comparing the root-mean-squared (RMS) of the two sets of
results since the RMS is more sensitive to outliers.
...
\begin{center}
    ...
    the LS periodogram, ACF, \citet{Mcquillan2013} ACF and GP period recovery
    methods.}
\begin{tabular}{lcccc}
    Method & MAD & MRD & RMS \\
    \hline
    LS periodogram & \pgramMAD\ days & \percentpgramMAD \% & \pgramRMS\ days\\
    ACF & \acfMAD\ days & \percentacfMAD \% & \acfRMS\ days \\
    ACF \citep{Mcquillan2013} & \telavivMAD\ days & \percenttelavivMAD \%
    & \telavivRMS\ days \\
...
These numbers are similar for the uninformative prior results which also
provides underestimated uncertainties.
...
method and a moderate one over our basic implementation of the ACF method.
We also find that our basic ACF method which is similar to methods implemented
in the literature, often slightly under-predicts the rotation period due to a
subtle feature of its peak detection algorithm.
Comparing our results with the results of the \citet{Aigrain2015} Tel Aviv
team who implement the method of \citet{Mcquillan2013}, a slightly different
version of the ACF method to the one we use, we find that their results are
slightly more precise than the GP method results overall.
They do however have a larger number of significant outliers.
We measured the rotation periods of \nkoimcq\ \kepler\ objects of interest
...
model. \tdmcomment{Can we make a statement something like ``we believe the method underestimates
