...
% because long-term trends unrelated to rotation still produced excess power at
% long periods, despite our attempts to remove these trends via filtering.
...
The issue of computational time is a personal one.
...
wherever possible.
Probabilistic inference also allows you to marginalise over, or ‘bypass’,
certain parameters.
For example, imagine trying to infer the distribution of rotation periods of a
sample of stars.
One could simply calculate rotation periods from the light curves and produce
a histogram of the rotation values.
This is `adding posteriors' which is not technically legal in probability
theory, you are only allowed to add (or sum) likelihoods.
In order to add likelihoods you have to jointly infer the rotation period of
each individual star and the parameters of the distribution, which you might
model as a Gaussian.
Here, the individual rotation periods of the stars are nuisance parameters:
you don’t care about their values, you only compute them in order to infer the
mean and variance of the period distribution.
Using hierarchical inference allows you to marginalise over individual
rotation periods and infer distribution parameters directly from the light
curves.
Many people are interested in the distribution of stellar ages across the
galaxy and rotation periods can be used as an age proxy via gyrochronology.
Probabilistic rotation period inference is therefore highly relevant to a
large number of galactic archaeology and exoplanet population studies which
require rotation period (or age) distributions.}
