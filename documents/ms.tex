...
\kepler-like light curves for solid-body rotators in \citet{Aigrain2015} using
...
approximate because is many cases the posterior PDFs of the parameters are
multi-modal.
...
It can only capture the posterior PDF for the periodic component of the
covariance matrix, not the rotation period of a physical star.
...
rotation periods which could, for example, reveal the period distribution of
(and therefore potentially the age distribution) of stars in the Milky Way.
There is still room for improvement of this model since that the uncertainty
estimates are not yet truly representative and it is still only an `effective'
model, not a physical one, but it is probabilistic, it provides more accurate
periods and, we argue, is the best method for rotation period inference
currently available.
