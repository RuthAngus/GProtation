...
In this work, we introduce an appropriate {\it effective} model: a Gaussian
    Process (GP) with a quasi-periodic covariance kernel function.
...
differential rotation on the stellar surface produce quasi-periodicity \vrcomment{\citep{dumusque2012}}.
...
and a choice of mother wavelet, assumed to \vrcomment{describe [VR: spelling]} the data
...
For example, the simplest and most commonly used kernel function \vrcomment{[VR: it's true that it's the most commonly used kernel, but not true that it's the `simplest' -- a constant or linear kernel is simpler; an exponential is as `simple' in terms of number of hyper-parameters; etc. There are of course other motivations for using SE kernel; see e.g. Rasmussen \& Williams pg.\ 14, although these details are probably not relevant to this paper. Maybe it's the simplest stationary kernel function that admits infinitely-differentiable function draws, although I stand to be corrected on that.]}, the `Squared
...
\end{equation}
Here $A \vrcomment{>0}$ is the amplitude of covariance \vrcomment{[VR: either say $A>0$ or use $A^2$ in the definition of this kernel; need to ensure positive values so QP covariance matrix has nonnegative eigenvalues i.e.\ is positive semi-definite.]}, $l$ is the length scale of
...
\Gamma^2 \sin^2\left(\frac{\vrcomment{\pi(x_i - x_j)}}{P}\right) \right].
\end{equation}
\vrcomment{[VR: there was a term missing from the above definition, viz.\ $(x_i - x_j)$ inside the $\sin()$. Without it, the $\sin()$ term reduces to a constant. Also, I changed $2\pi$ to $\pi$: if you define it with $2\pi$ then the period will differ from the physical period by a factor of $2$, given the squared term modifying the sine wave: ${\sin ^2}(\frac{\theta }{2}) = \frac{1}{2} - \frac{1}{2}\cos \theta $. This can be checked by plotting some covariance kernels with large $l$ and noting that peaks appear with expecting spacing $P$ only if you remove the factor of $2$ that originally appeared in the definition.]}   It is the product of the SE kernel function, which describes the overall
...
If $\Gamma$ is very \vrcomment{large}, only points almost exactly one period away are
tightly correlated and points that are slightly more or less than one period
away are very loosely correlated.
If $\Gamma$ is \vrcomment{small}, points separated by one period are tightly
correlated, and points separated by slightly more or less are still highly
correlated, although less so. \vrcomment{[VR: behaviour for large vs.\ small values of $\Gamma$ was described backwards; i.e.\ correct explanation is that large $\Gamma$ leads to only points $\sim$~one period apart being tightly correlated, with weak correlations for other separations. Another way of expressing this would be to say that large values of $\Gamma$ lead to periodic variations with increasingly complex harmonic content. Or in non-technical terms: more bumps and wiggles per function repetition.]}
...
term added to the diagonal elements of the covariance matrix. \vrcomment{[VR: I think it might be clearer if you just included this white noise term directly in the definition of the kernel, especially for people who might not be familiar with GPs and for whom it won't necessarily be obvious how the white noise term connects to the rest of the kernel. I.e.\ I'd suggest you simply defined the kernel as below.]}
\begin{equation}
\label{eq:QP2}
\vrcomment{k_{i,j} = A \exp \left[-\frac{(x_i - x_j)^2}{2l^2} -
\Gamma^2 \sin^2\left(\frac{\pi\vrcomment{(x_i - x_j)}}{P}\right) \right] + \sigma^2\delta_{ij}.}
\end{equation}
...
and the results are discussed in \textsection \ref{sec:discussion}. \vrcomment{[VR: the order doesn't make sense. As things stand, you've laid it out as \textsection3.1,\textsection2, then  \textsection3.3. Presumably should be \textsection2,\textsection3.1, \textsection3.3 -- and \textsection3.2 is missing].}

\section{GP Rotation Period Inference}
\label{sec:method}

In order to recover a stellar rotation period from a light curve using a
quasi-periodic Gaussian process (QP-GP), we sample the following posterior PDF:
\begin{equation}
\label{eq:posterior}
...
\begin{itemize}
    \item The likelihood \vrcomment{evaluation [VR: spelling]} is computationally expensive;
...
Gaussian \vrcomment{uncertainties [VR: spelling]} on the data. \vrcomment{VR: the above expression is incorrect. It should read:
\begin{equation}
\ln \mathcal{L} =  - \frac{1}{2}\sum\limits_{n = 1}^N {\frac{{{{({y_n} - \mu )}^2}}}{{\sigma _n^2}} - } \frac{1}{2}\sum\limits_{n = 1}^N {\ln (2\pi \sigma _n^2)},
\end{equation}
or equivalently
\begin{equation}
\ln \mathcal{L} =  - \frac{1}{2}\sum\limits_{n = 1}^N {\left\{ {\frac{{{{({y_n} - \mu )}^2}}}{{\sigma _n^2}} + \ln (2\pi \sigma _n^2)} \right\}}.
\end{equation}
The problem with the expression as written above is that either the second term must be included in the summation, assuming $\sigma_n\neq\sigma$, in which case the factor of $N$ should not appear in the second term; or if $\sigma_n=\sigma$~$\forall n$ (which now no longer represents the general case), then the factor of $N$ is correct, but the subscript $n$ should be removed from $\sigma_n^2$.}
...
\end{equation}
\label{eq:lhf1}
\vrcomment{(VR: why not just write $\frac{N}{2}\log{2\pi}$ instead of `constant'?)}~where $\bf{r}$ is the vector of residuals and $\bf{C}$ is the covariance
matrix,
\begin{eqnarray}
...
Evaluating this \vrcomment{likelihood [VR: spelling]} for a large number of points can be computationally
...
Managing this flexibility to focus on reliably
retrieving the period parameter requires imposing priors 
on the non-period GP parameters.
...
\vrcomment{VR: in my opinion, this is the weakest part of the paper. I don't agree that there are no \emph{a priori} reasons to believe that any of the hyperparameters should take values in any specific range, and the phrase `somewhat subjective art' seems likely to set alarm bells ringing for referees/readers. Going through the hyperparameters in turn, the following things come to mind.
...
\vrcomment{probabilistically [VR: spelling]} justified inference scheme rather than
...
We run \vrcomment{50} steps of the sampler at a time, checking for convergence after
each iteration, up to a maximum of \vrcomment{50} iterations.
We declare convergence if the total effective chain length is at least
8$\times$ the maximum autocorrelation time.
When convergence is achieved, we \vrcomment{discard} the first two autocorrelation lengths in
...
full set of 1000 light curves. \vrcomment{[VR: a few months ago I did a bunch of tests with kernels I constructed specifically to account for differential rotation. I need to dig up that work but I remember the clear conclusion at which I arrived was that there seems to be an unbreakable degeneracy between any extra hyperparameter(s) to account for differential evolution, and the standard QP hyperparameters. This might change if enforcing $l\gg P$; I can't remember. Anyway, I mention this now just as a little warning that we might not be able to count on the GP approach yielding many useful insights as far as differential rotation goes.]}
...
potentially requiring modifications to these priors \vrcomment{[VR: might want to reword or remove these remarks; see my earlier comments]}
...
The marginal posterior distributions of the QP kernel hyperparameters\vrcomment{,} for the
...
\caption{Marginal posterior PDFs of the QP GP model parameters, fit to the
    ...
    The green line in the period panel shows the injected period. \vrcomment{[VR: might it be possible to make the green line darker? It took me a little while to find it.]}
...
at long periods---a problem further exacerbated by instrumental noise. 
\tdmcomment{I don't quite understand this---what do we mean here by 
``instrumental noise'' as distinct from long-term trends?}
...
\tdmcomment{What are these numbers for the uninformed-prior experiment?}
...
A perfect physical model of the star would still produce better results with
more representative uncertainties. \tdmcomment{Not sure saying ``a perfect
model would give better results'' is worth saying...} \vrcomment{VR: I agree this is probably not worth saying.}

The QP kernel function represents a simplistic effective model of a stellar
light curve.
It adequately describes the data, captures that all-important periodic quality \vrcomment{[VR: I think this is a bit vague...a sinusoidal model would capture `periodic quality'. Maybe say something like: `it manages to describe a wide range of quasi-periodic behavior'.]}
and is relatively simple, with only a few hyperparameters.
It also satisfies the requirement to produce positive semi-definite covariance
matrices. \vrcomment{[VR: I don't think it's worth mentioning the PSD thing. It's not as though this is some advantage the QP kernel has over any other kernel function.]}
Whilst the QP kernel function evidently captures the periodic qualities of
light curves adequately, it is still a somewhat arbitrary choice \vrcomment{[VR: repetitive...maybe just say 'Nevertheless, it is still a somewhat arbitrary choice.']}.
Another valid choice would be a squared cosine function multiplied by a
squared exponential,
\begin{equation}
k_{i,j} = A \exp \left(-\frac{(x_i - x_j)^2}{2l^2}\right)
\cos\left(\frac{\pi\vrcomment{(x_i - x_j)}}{P}\right)
\end{equation}
\label{eq:cos_kernel}
\vrcomment{VR: missing $(x_i - x_j)$ term added above, and factor of $2$ removed.} This function produces a positive semi-definite matrix and has the $P$
...
alternative kernel functions could be tested.} \vrcomment{[VR: this is something I've started exploring using MultiNest. Computation times seem quite manageable.]}
...
\item \vrcomment{[VR: for the two points above, you might want to reword slightly depending on the extent to which you want to try to (i) incorporate my suggestions re prior constraints etc.\ etc.\ vs.\ (ii) deferring this to the future paper, but at least alluding to the possibilities and results. For example I think one needn't try to `design' a physically-motivated kernel -- rather it seems not too difficult just to come up with physically-motivated priors for the QP kernel parameters. I've tried to design a physically motivated kernel to capture differential rotation but as noted earlier, that came with its own slew of degeneracies and other problems...more on this another time.]}
...
approximate because in many cases the posterior PDFs of the parameters are
multi-modal.
...
where part of this work was completed. VR thanks Merton College and the National Research Foundation of South Africa for financial support.
